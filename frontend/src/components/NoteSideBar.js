...
function NoteSideBar() {
  ...
  allNotes.map.forEach(function (note) {
    ...
    let className = `transition-colors duration-150 h-24 cursor-pointer border
     text-left mx-7 my-5 px-4 py-4 rounded-lg `;
    ...
    if (note.body.length > 0) {
      displayBody =
        note.body.substr(0, 20) + (note.body.length > 20 ? '...' : '');
    }
    allNoteElements.push(<div
      key={note._id}
      className={className}
      onClick={() =>
        setChoosenNoteId(note._id)}>
      <span className="text-base font-bold">{displayTitle}</span>
      <div className="mt-0.5">
        <span className="w-20 inline-block">
          {dateFormater.format(createdAt)}
        </span>
        <span
          className={`inline-block ${isChoosenNote ? '' : 'text-gray-500'}`}>
          {displayBody}
        </span>
      </div>
    </div>);
